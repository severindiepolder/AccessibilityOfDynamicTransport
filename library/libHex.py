...
def hexagonalGrid(bbox, cell, gtfsDBStops, distanceS, city,check4Stops):
    ...
    for x in range(0, x_count):
        for y in range(0, y_count + 1):
            ...
            if ((StopsCount > 0) or (check4Stops == False)):
                ...
                count_ins += 1
            if count_ins % 10 == 0:
                print('{0:.1f}%, tot = {1}, inserted = {2}'.format(
                    100. * (float(x * y_count) + float(y)) / (x_count * y_count), (float(x * y_count) + float(y)),
                    count_ins), end="\r")
...
def reduceGeojsonInShell(hexs, field, color=colorIso, shell=shellIso):
    ...
    for i, lim in enumerate(shell[:-1]):
        ...
        if len(find) > 0:
            ...
            map_osm.choropleth(geojson, fill_color=color[i], fill_opacity=0.6, line_color=color[i], line_weight=2,
                               line_opacity=0, )
